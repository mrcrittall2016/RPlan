<?php
    ...
    foreach ($smiles as $value){
        ...
        $rows = Database::query("SELECT id FROM `inthev8_main_rplanner` . `cmpds` WHERE smiles = ?", $value);
        ...
        // Need to store compound as unique
            ...
            // Store compound 
            $result = Database::query("INSERT IGNORE INTO `inthev8_main_rplanner`. `cmpds` (smiles) 
            VALUES(?)", $value);
    ...
    $result = Database::query("INSERT IGNORE INTO `inthev8_main_rplanner`. `reactions` (cmpds_id, yields, steps, quantity, units, start, Scheme_name, username) 
